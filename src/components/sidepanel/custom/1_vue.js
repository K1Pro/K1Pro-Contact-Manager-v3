export default {
  ...
  template: /*html*/ `
    <template v-if="slctdCntct && userData?.AppPermissions.ContactManager[0] == 'bundle_insurance'">
        ...
        <div class="policy-info-title">
          <div class="policy-info-title-grid-container">
            ...
            <div class="policy-info-title-grid-item2">
              <button @click="newPolicy" ><i class="fa-solid fa-square-plus"></i></button>
            </div>
          </div>
        </div>
        ...
        <div v-for="policyInfo, policyInfoIndex in slctdCntct.Custom1" class="policy-info-policy" :style="{ 'background-color': policyInfoIndex % 2 ? 'lightblue' : 'white'}">
          ...
          <div v-for="policyInfoInputs in accountSettings.Custom1">
            ...
            <template v-if="policyInfoInputs.type == 'select'">
              <select @change="updatePolicy($event, policyInfoIndex, policyInfoInputs.value)" :style="{ 'background-color': policyInfoIndex % 2 ? 'lightblue' : 'white', 'border': policyInfoIndex % 2 ? '1px solid gray' : '1px solid lightgray' }">
